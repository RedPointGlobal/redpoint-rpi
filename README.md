...
- **Data Warehouses**
    - Databases on any of the following platforms: ```AzureSQLDatabase```, ```AmazonRDSSQL```, ```GoogleCloudSQL```, ```SQLServer on VM```, ```AzureDatabaseMySQL```, ```AzureDatabasePostgreSQL```, ```Snowflake```, ```PostgreSQL```
...
- Provide the connection details for:
   ...
   - Data warehouse server (If using Redshift, Databricks, or BigQuery)
...
If your [datawarehouse](https://docs.redpointglobal.com/rpi/supported-connectors#Supportedconnectors-Databaseplatforms) is one of ```Redshift```, ```Databricks``` or ```BigQuery```, open the ```values.yaml``` file and provide connection details relevant to your chosen provider.
...
This chart deploys RPI with a single replica for each service. To achieve high availability you can use one of the options below configured in the ```values.yaml``` file.
...
```
FROM rg1acrpub.azurecr.io/docker/redpointinteraction/prod/redpoint-interaction-execution-service-unixodbc:7.6.2025-0429-1556
...
RPI services expose ```/metrics``` endpoint for scrapping using Prometheus. For the web services, this endpoint provides a collection of default .NET metrics. Other services expose custom metrics specific to their core functionalities. Enabling custom metrics assumes that you already have Prometheus running in your cluster and configured to scrape metrics targeting the rpi namespace.
...
- **Execution Service:** Displays the number of activities currently running, the total executed since startup, and the maximum number of activities it is configured to handle. You can create dashboards in Grafana for visualization with the following metrics:

```
execution_max_thread_count
execution_total_executing_count
execution_client_jobs_executing_count
execution_tasks_executing_count
execution_workflows_executing_count
execution_client_jobs_completed_count
execution_tasks_completed_count
execution_workflows_completed_count
execution_workflows_suspended_count
```

- **Node Manager:** Tracks the number of activities allocated to execution services and the number of triggers it fires. You can create dashboards in Grafana for visualization with the following metrics:

```
node_manager_activities_allocated_count
node_manager_tasks_allocated_count
node_manager_triggers_fired_count
```

- **Queue Reader:** Counts the number of items processed in its various queues, giving visibility into queue consumption and throughput. You can create dashboards in Grafana for visualization with the following metrics:

```
web_form_queue_drainer_5e8dc5c7_6807_48c5_b984_98d4a9c8cb3b
web_form_web_form_event_processor_5e8dc5c7_6807_48c5_b984_98d4a9c8cb3b
web_form_web_form_submission_importer_5e8dc5c7_6807_48c5_b984_98d4a9c8cb3b
web_form_web_form_notification_processor_5e8dc5c7_6807_48c5_b984_98d4a9c8cb3b
realtime_profile_queue_drainer_5e8dc5c7_6807_48c5_b984_98d4a9c8cb3b
realtime_profile_realtime_profile_database_importer_5e8dc5c7_6807_48c5_b984_98d4a9c8cb3b
realtime_events_queue_drainer_5e8dc5c7_6807_48c5_b984_98d4a9c8cb3b
realtime_events_event_audit_processor_5e8dc5c7_6807_48c5_b984_98d4a9c8cb3b
realtime_events_interaction_event_processor_5e8dc5c7_6807_48c5_b984_98d4a9c8cb3b
realtime_events_realtime_event_processor_5e8dc5c7_6807_48c5_b984_98d4a9c8cb3b
realtime_events_event_result_processor_5e8dc5c7_6807_48c5_b984_98d4a9c8cb3b
```
