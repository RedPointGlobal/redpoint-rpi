...
2. Test with a dry run:
   ```bash
   argocd app diff rpi-staging --local chart --values ../deployments/staging.yaml
   ```
...
cloudIdentity:
  ...
  secretsManagement:
    enabled: true
    secretsProvider: awssecretsmanager    # or keyvault
...
When using `valueFiles: [../deployments/production.yaml]`, the path is relative to the chart directory. The `../` prefix navigates up to the repo root and then into `deployments/`.
