...
| `deployments/values-reference.yaml` | Complete reference of every possible key and its default | No (read-only reference) |
| `deployments/dev.yaml` | Your dev overrides | Yes |
| `deployments/staging.yaml` | Your staging overrides | Yes |
| `deployments/production.yaml` | Your production overrides | Yes |
...
│   ├── values-reference.yaml     # Full reference of all keys
│   ├── dev.yaml
│   ├── staging.yaml
│   └── production.yaml
...
1. Copy the appropriate environment file from `deployments/` as your starting point.
...
```bash
# Dev
helm upgrade --install rpi ./chart -f ./deployments/dev.yaml -n rpi-dev --create-namespace

# Staging
helm upgrade --install rpi ./chart -f ./deployments/staging.yaml -n rpi-staging --create-namespace

# Production
helm upgrade --install rpi ./chart -f ./deployments/production.yaml -n rpi-production --create-namespace
```
...
Open `deployments/values-reference.yaml` and scroll to the `advanced:` section at the bottom. Every key is documented with its default value shown as commented-out YAML.
...
2. **Start from an example.** Copy `deployments/production.yaml` or `deployments/staging.yaml` and fill in your values.
...
4. **Test with `helm template`** to verify your rendered manifests match the previous deployment:
   ...
   ```bash
   helm template rpi ./chart -f ./deployments/production.yaml > rendered.yaml
   ```
