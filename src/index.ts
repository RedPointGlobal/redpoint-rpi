...
import { template, type TemplateInput } from "./tools/template.js";
...
const TOOLS = [
  {
    name: "rpi_validate",
    description:
      "Validate an RPI Helm chart values file against the JSON Schema and heuristic rules. " +
  ...
  },
  {
    name: "rpi_generate",
    description:
      "Generate a starter RPI overrides YAML from structured parameters. " +
  ...
  },
  {
    name: "rpi_template",
    description:
      "Render Helm templates for the RPI chart using provided values YAML. " +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        showOnly: {
          type: "string",
          description:
            "Render only this template (relative to chart/, e.g., 'templates/deploy-realtimeapi.yaml')",
        },
        namespace: {
          type: "string",
  ...
  },
  {
    name: "rpi_explain",
    description:
      "Explain a specific values.yaml key path. Returns the type, description, default, " +
    ...
    inputSchema: {
      ...
      properties: {
        keyPath: {
          type: "string",
          description:
            "Dot-delimited key path (e.g., 'secretsManagement.provider')",
        },
      },
      required: ["keyPath"],
    },
  },
  {
    name: "rpi_status",
    description:
      "Get the status of an RPI deployment in a Kubernetes namespace. " +
  ...
  },
  {
    ...
    description:
      "Diagnose issues with an RPI deployment. Checks pod health, secrets, ingress, " +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        symptom: {
          type: "string",
          description:
            "Optional symptom keyword: 'crashloop', 'pending', 'imagepull'",
        },
      },
    },
  },
  {
    ...
    description:
      "Search the official RPI product documentation at docs.redpointglobal.com/rpi. " +
    ...
    inputSchema: {
      ...
      properties: {
        query: {
          type: "string",
          description:
            "Search keywords (e.g., 'realtime cache setup', 'queue provider rabbitmq', 'authentication OIDC')",
        },
        maxResults: {
          type: "number",
  ...
  },
  {
    name: "rpi_docs_fetch",
    description:
      "Fetch a specific page from the RPI documentation site. Use this when you know the exact " +
    ...
    inputSchema: {
      ...
      properties: {
        url: {
          type: "string",
          description:
            "Full URL (https://docs.redpointglobal.com/rpi/...) or just the slug (e.g., 'admin-authentication')",
        },
      },
      required: ["url"],
    },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    ...
    case "rpi_generate": {
      ...
      return {
        content: [{ type: "text" as const, text: yamlOutput }],
      };
    }

    case "rpi_template": {
      const input = args as unknown as TemplateInput;
    ...
    case "rpi_explain": {
      const input = args as { keyPath: string };
    ...
    case "rpi_status": {
      const input = args as { namespace?: string };
    ...
    case "rpi_troubleshoot": {
      const input = args as { namespace?: string; symptom?: string };
    ...
    case "rpi_docs_search": {
      const input = args as { query: string; maxResults?: number };
