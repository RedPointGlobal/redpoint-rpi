...
function heuristicChecks(
  ...
  const deployment = global?.deployment as Record<string, unknown> | undefined;
  if (deployment?.mode === "demo") {
    const secretsMgmt = values.secretsManagement as Record<string, unknown> | undefined;
  ...
  const secretsMgmt = values.secretsManagement as Record<string, unknown> | undefined;
  const cloudIdentity = values.cloudIdentity as Record<string, unknown> | undefined;
  if (
    secretsMgmt?.provider &&
  ...
  const rtapi = values.realtimeapi as Record<string, unknown> | undefined;
  const cacheProvider = rtapi?.cacheProvider as Record<string, unknown> | undefined;
  if (cacheProvider?.enabled && cacheProvider?.provider === "mongodb") {
    const mongo = cacheProvider.mongodb as Record<string, unknown> | undefined;
 ...
 */
export function validate(valuesInput: string): { results: ValidationResult[]; parsed: boolean } {
  ...
  try {
    values = yaml.load(yamlContent) as Record<string, unknown>;
