import { isClusterAvailable, kubectlGet, kubectlDescribe, kubectlLogs, kubectlEvents, } from "../lib/kubectl.js";
/**
 * Known symptom patterns and their diagnostic procedures.
 */
const SYMPTOM_HANDLERS = {
        ...
        try {
            ...
            for (const line of pendingPods) {
                ...
                try {
                    ...
                    const events = eventsIdx >= 0 ? desc.slice(eventsIdx) : "(no events)";
                    findings.push({
    ...
    imagepull: (ns, findings) => {
        try {
            ...
            const errPods = pods
                .split("\n")
                .filter((l) => l.includes("ImagePullBackOff") || l.includes("ErrImagePull"));
 ...
 */
function generalDiagnostics(ns, findings) {
    // Check pods
    try {
        const pods = kubectlGet("pods", ns);
    ...
    try {
        ...
        if (!secrets.includes("redpoint-rpi")) {
            findings.push({
                category: "secrets",
                severity: "warning",
                message: "No secret matching 'redpoint-rpi' found. RPI services may fail to start.",
 ...
 */
export function troubleshoot(namespace, symptom) {
            ...
            findings: [
                {
                    category: "cluster",
                    severity: "critical",
                    message: "No Kubernetes cluster is reachable. Ensure kubectl is configured and the cluster is accessible.",
