 ...
 */
function deepDiff(defaultObj, userObj, prefix = "") {
    const diffs = [];
    for (const key of Object.keys(userObj)) {
        ...
        if (defaultVal === undefined) {
            ...
            diffs.push(...deepDiff(defaultVal, userVal, fullPath));
        }
        else {
            // Leaf comparison
 ...
 */
export function migrate(valuesInput) {
    const warnings = [];
    // Parse user's v7.6 values
    let userValues;
    if (existsSync(valuesInput)) {
        userValues = yaml.load(readFileSync(valuesInput, "utf-8"));
    }
    else {
        userValues = yaml.load(valuesInput);
    }
    ...
    for (const { path, value } of diffs) {
        ...
        const prefixMatch = Object.entries(KEY_RENAMES).find(([oldPrefix, newPrefix]) => path.startsWith(oldPrefix + ".") && newPrefix !== "__REMOVED__" && newPrefix !== "__MOVED_TO_TLS_ARRAY__");
        if (prefixMatch) {
        ...
        const removedParent = Object.entries(KEY_RENAMES).find(([oldPrefix, newPrefix]) => path.startsWith(oldPrefix + ".") && newPrefix === "__REMOVED__");
        if (removedParent) {
            continue;
        }
    ...
    const v76Images = userValues?.global?.deployment?.images;
    if (v76Images?.tag) {
    ...
    if (userValues?.cloudIdentity?.secretsManagement?.enabled) {
        warnings.push("secretsManagement has moved from cloudIdentity.secretsManagement to top-level secretsManagement. " +
            "Review the generated output to ensure provider settings are correct.");
    }
    const dbPassword = v77?.databases?.operational?.server_password;
    if (dbPassword && !dbPassword.includes("SECRET") && !dbPassword.includes("CHANGE_ME")) {
        warnings.push("Database password detected in plain text. Consider using secretsManagement (sdk or csi) " +
            "instead of embedding passwords in your values file.");
    }
    if (userValues?.ingress?.className === "nginx-redpoint-rpi") {
        warnings.push("ingress.className default changed from 'nginx-redpoint-rpi' to the release namespace. " +
