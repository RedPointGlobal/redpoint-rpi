...
export function generate(input) {
    ...
    if (input.database) {
        values.databases = {
            operational: {
                ...
                pulse_logging_database_name: input.database.pulse_logging_database_name ?? "<my-logging-database-name>",
                encrypt: true,
            },
        };
    }
    ...
    if (input.platform === "azure") {
        cloudIdentity.azure = {
            managedIdentityClientId: ciInput?.azure?.managedIdentityClientId ?? "<your-workload-identity-client-id>",
            tenantId: ciInput?.azure?.tenantId ?? "<your-azure-tenant-id>",
        };
    }
    else if (input.platform === "google") {
        cloudIdentity.google = {
            serviceAccountEmail: ciInput?.google?.serviceAccountEmail ??
        ...
        cloudIdentity.amazon = {
            roleArn: ciInput?.amazon?.roleArn ?? "arn:aws:iam::<account-id>:role/<role-name>",
    ...
    if (input.realtimeapi) {
        ...
        if (input.realtimeapi.cacheProvider) {
            ...
            if (input.realtimeapi.cacheProvider === "mongodb") {
                cp.mongodb = {
                    connectionString: input.realtimeapi.mongoConnectionString ??
                        "<your-mongodb-connection-string>",
                    databaseName: "RPIRealtimeCache",
                };
            }
            else if (input.realtimeapi.cacheProvider === "redis" ||
                input.realtimeapi.cacheProvider === "azureredis") {
                cp.redis = {
                    connectionString: input.realtimeapi.redisConnectionString ??
