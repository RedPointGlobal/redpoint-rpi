...
function lineDiff(original, modified) {
    ...
    const origContentCount = {};
    const modContentCount = {};
...
// Main
// ---------------------------------------------------------------------------
export function migrateTemplates(templatesPath) {
    const warnings = [];
    // Validate path
    ...
    for (const filename of userFiles) {
        ...
        if (filename === "_helpers.tpl") {
            ...
            warnings.push("You have customizations in _helpers.tpl. This file was extensively rewritten in v7.7. " +
    ...
    for (const stockFile of stockFilenames) {
        if (!userFiles.includes(stockFile)) {
            warnings.push(`Stock v7.6 template ${stockFile} is missing from your templates directory. ` +
    ...
    // Overall guidance
    let overallGuidance = "## Migration Steps\n\n" +
