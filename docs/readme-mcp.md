...
**Option B: Web UI (guided plan)**

Use the **Agentic** tab in the Web UI to describe your deployment. The assistant generates a complete deployment plan including:

- Infrastructure provisioning command (Bicep)
- Helm overrides file (downloadable)
- Key Vault secrets checklist
- Preflight checks
- Cluster installation command

You then run the commands in your own environment.

### Security model

| Concern | How it's handled |
|---------|-----------------|
| App secrets | Key Vault + Workload Identity (SDK mode). The assistant never sees values. |
| Image pull secret | You pre-create it. The assistant verifies it exists. |
| TLS certificate | Key Vault → CSI Secret Store sync → K8s Secret `ingress-tls` |
| Database password | You type it directly into the `az` CLI — it doesn't pass through the assistant |
