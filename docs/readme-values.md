...
| Your `overrides.yaml` | Your environment-specific configuration | Yes (generated by the Web UI or CLI) |
| Your `secrets.yaml` | Kubernetes Secret manifest with sensitive values | Yes (generated by the CLI) |
...
There are two ways to generate your overrides file. Both produce the same output: a clean `overrides.yaml` with only your customizations.

### Option A: Web UI (recommended)

Use the **Helm Assistant Web UI** at [rpi-helm-assistant.redpointcdp.com](https://rpi-helm-assistant.redpointcdp.com) to build your overrides visually. Select your platform, configure features, and download the YAML. The "Start Here" page walks you through the full workflow.

### Option B: Interactive CLI

Run the **Interaction CLI** for a guided terminal experience. It walks you through every configuration decision and writes `overrides.yaml`, `secrets.yaml`, and `prereqs.sh` in one pass.
...
| Deployment mode | `standard` (existing databases), `advanced` (enterprise features), or `demo` (self-contained) |
...
The interactive CLI prompts for passwords, API keys, and connection strings during generation and writes them directly to `secrets.yaml`.

If you used the Web UI to generate your overrides, run the CLI `secrets` command to generate `secrets.yaml` from your overrides file. It reads the configuration to determine which credentials are needed and prompts only for those:
...
Both `overrides.yaml` and `secrets.yaml` are plain YAML files. Open them in any editor, correct any values, and save before deploying. The generated files are yours to review and adjust.

### Deploying

The CLI `deploy` command handles namespace creation, secrets application, and Helm install/upgrade with live rollout monitoring:

```bash
bash deploy/cli/interactioncli.sh deploy -f overrides.yaml -n redpoint-rpi
```

To preview rendered manifests without deploying:

```bash
bash deploy/cli/interactioncli.sh deploy -f overrides.yaml --dry-run
```
...
| Command | Description |
|---------|-------------|
| *(no args)* | Full interactive setup (generates overrides + secrets + prereqs) |
| `secrets -f <file>` | Generate secrets.yaml from an existing overrides file |
| `deploy -f <file>` | Deploy to Kubernetes (prereqs + helm install/upgrade) |
| `deploy -f <file> --dry-run` | Render templates without deploying |
| `status -n <ns>` | Show pod, service, and ingress status |
| `troubleshoot -n <ns>` | Diagnose common deployment issues |
| `-a <feature>` | Add a feature to an existing overrides file |
...
These are **not** starting points to copy and edit. Use the Web UI or Interaction CLI to generate your overrides. The examples are useful for reviewing what was generated or understanding what a complete override looks like for a given platform.
...
│   │   ├── deploy.sh             # Helm install/upgrade with rollout monitoring
...
**Web UI:** The Helm Assistant Web UI at [rpi-helm-assistant.redpointcdp.com](https://rpi-helm-assistant.redpointcdp.com) provides a visual interface for generating overrides, validating configurations, migrating from v7.6, and browsing documentation. The CLI package is also available for download from the Web UI.
...
See the [Migration Guide](migration.md) for step-by-step instructions on moving from v7.6 to v7.7, including automated migration with the Interaction Helm Assistant MCP tool.
